    ...
    $.fn.waterTank = function( options, extra ) {	
        ...
        var speed = 500;
       	...
       	function _updateTank($that, value, valortanque){
			...
			$that.find('._percentageTag').text(value + 'L');
			$that.find('._reflect').stop().animate({
				height: ($that.find('._tank').height() * value / valortanque) / 2 + 20 ,
				bottom: -($that.find('._tank').height() * value / valortanque) / 2 - 20,
			},
				speed,
				"_easeOutQuint"
			);
